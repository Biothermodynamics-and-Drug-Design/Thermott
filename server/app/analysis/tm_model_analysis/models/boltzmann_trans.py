...
def TmModelFunc(T, Tm, k,yf, yfs, yu, yus):
    T = np.array(T) + 273.15
    Tm = Tm + 273.15
    a = yf+yfs*(T-Tm)
...
def PerformInitialFit(data_x, data_y):
    ...
    p0 = TmModelParams_BoltzmannTrans(Tm = np.average(data_x_fit), k=1, yf = i_yf , yfs = i_yfs , yu = i_yu, yus = i_yus)
...
def TmModelFit(data_x_fit, data_y_fit, p0):
    ...
    initial_params = [p0.Tm, p0.k, p0.yf, p0.yfs, p0.yu, p0.yus]
    bounds = [[],[]]
    bounds[0] = [np.min(data_x_fit), 0.001, -5, -10, -5, -10]
    #Testing: limit coeffs to negative numbers only
    #bounds[1] = [np.max(data_x_fit), 350_000, 80_000, 100, 10, 130, 10]
    bounds[1] = [np.max(data_x_fit), 10, 100, -0.001, 130, -0.001]
    ...
    params = TmModelParams_BoltzmannTrans(Tm=p[0],k=p[1], yf=p[2], yfs=p[3], yu=p[4], yus=p[5])
...
def TmModelFitRange(data_x_fit, data_y_fit):
    ...
    p0 = TmModelParams_BoltzmannTrans(Tm=np.average(data_x_fit), k = 1, yf=p[0], yfs=p[1], yu=p[2], yus=p[3])
